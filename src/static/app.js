      ...
      Object.entries(activities).forEach(([name, details]) => {
          ...
          <ul class="participants-list">
            ${details.participants.map(email => `
              <li class="participant-item">
                ...
                <button type="button" class="delete-participant-button" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">×</button>
              </li>
            `).join('')}
          </ul>
        `;
        ...
        activityCard.querySelectorAll('.delete-participant-button').forEach((button) => {
          button.addEventListener('click', () => {
            deleteParticipant(button.dataset.activity, button.dataset.email);
          });
        });
